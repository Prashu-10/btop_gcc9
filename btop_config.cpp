...
namespace Config {
	...
	bool _locked(const std::string& name) {
    // Replace atomic_wait with a spinlock or other synchronization mechanism
    while (writelock.load() == true) {
        // Spinlock or sleep
    }

    // Replace rng::find_if with std::find_if
    if (!write_new && std::find_if(descriptions.begin(), descriptions.end(), 
        [&name](const auto& a) { return a.at(0) == name; }) != descriptions.end()) {
        write_new = true;
    }
	...
	bool toggle_box(const std::string& box) {
		auto old_boxes = current_boxes; // Assuming current_boxes is a std::vector<std::string>

		// Replace rng::find with std::find
		auto box_pos = std::find(current_boxes.begin(), current_boxes.end(), box);

		if (box_pos == current_boxes.end())
			current_boxes.push_back(box); // Add the box if it doesn't exist
		else
			current_boxes.erase(box_pos); // Remove the box if it exists

		// Build the new_boxes string
		std::string new_boxes;
		if (!current_boxes.empty()) {
			for (const auto& b : current_boxes) {
				new_boxes += b + ' ';
			}
			new_boxes.pop_back(); // Remove the trailing space
		}

		// Get the minimum size required for the new layout
		...
		if (Term::width < min_size.at(0) || Term::height < min_size.at(1)) {
			current_boxes = old_boxes; // Revert changes if the terminal is too small
			return false;
    }
