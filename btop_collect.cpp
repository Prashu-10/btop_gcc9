...
namespace Cpu {
	...
	bool get_sensors() {
		...
		if (not got_coretemp or core_sensors.empty()) {
			...
			// Sort core_sensors using std::less
			std::sort(core_sensors.begin(), core_sensors.end(), std::less{});

			// Stable sort core_sensors based on size
			std::stable_sort(core_sensors.begin(), core_sensors.end(), [](const auto& a, const auto& b) {
	...
	auto collect(bool no_update) -> net_info& {
		...
		if (selected_iface.empty() or not v_contains(interfaces, selected_iface)) {
			...
			if (not config_iface.empty() and v_contains(interfaces, config_iface)) selected_iface = config_iface;
				...
				std::sort(sorted_interfaces.begin(), sorted_interfaces.end(), [&](const auto& a, const auto& b) {
					return cmp_greater(
...
namespace Proc {
	...
	void _collect_details(const size_t pid, const uint64_t uptime, vector<proc_info>& procs) {
		...
		auto p_info = std::find_if(procs.begin(), procs.end(), [&](const proc_info& p) {
			return p.pid == pid;
		});
		...
		if (detailed.parent.empty()) {
			auto p_entry = std::find_if(procs.begin(), procs.end(), [&](const proc_info& p) {
				return p.pid == detailed.entry.ppid;
			});
	...
	auto collect(bool no_update) -> vector<proc_info>& {
		...
		//* ---------------------------------------------Collection start----------------------------------------------
			...
			for (const auto& d: fs::directory_iterator(Shared::procPath)) {
				...
				auto find_old = std::find_if(current_procs.begin(), current_procs.end(), [&](const proc_info& p) {
					return p.pid == pid;
				});
				bool no_cache{};
				...
				if (no_cache) {
					...
					new_proc.name_offset = std::count(new_proc.name.begin(), new_proc.name.end(), ' ');
			...
			auto eraser = std::remove_if(current_procs.begin(), current_procs.end(), [&](const auto& element) {
				return !v_contains(found, element.pid);
			});
			current_procs.erase(eraser, current_procs.end());
		...
		if (tree and (not no_update or should_filter or sorted_change)) {
			...
			if (auto find_pid = (collapse != -1 ? collapse : expand); find_pid != -1) {
				auto collapser = std::find_if(current_procs.begin(), current_procs.end(), [&](const proc_info& p) {
					return p.pid == find_pid;
				});
			...
			std::stable_sort(current_procs.begin(), current_procs.end(), [](const proc_info& a, const proc_info& b) {
				return a.ppid < b.ppid;
			});

			//? Start recursive iteration over processes with the lowest shared parent pids
			// Define a custom comparator for ppid
			auto ppid_comp = [](const proc_info& a, int ppid) { return a.ppid < ppid; };

			// Find the range of elements with the same ppid as the first element
			auto range = std::equal_range(
				current_procs.begin(), current_procs.end(), current_procs.at(0).ppid, ppid_comp
			);
			...
			std::sort(current_procs.begin(), current_procs.end(), [](const proc_info& a, const proc_info& b) {
			...
			if (locate_selection) {
				auto it = std::find_if(current_procs.begin(), current_procs.end(), [&](const proc_info& p) {
					return p.pid == Proc::selected_pid;
				});

				if (it != current_procs.end()) {
					int loc = it->tree_index;
				} else {
					// Handle the case where the selected PID is not found
					throw std::runtime_error("Selected PID not found in current_procs");
				}
