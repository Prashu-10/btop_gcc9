   ...
   const array<array<string, 3>, 3> menu_normal = {
	...
	int mainMenu(const string& key) {
		...
		if (retval == Changed) {
			...
			for (int i = 0; i < 3; ++i) {
				...
				const auto& menu = (!tty_mode && i == selected) ? menu_selected[i] : menu_normal[i];
				const auto& colors = (i == selected) ? colors_selected : colors_normal;

				if (redraw) {
					mouse_mappings["button_" + std::to_string(i)] = {cy, Term::width / 2 - menu_width[i] / 2, 3, menu_width[i]};
				}
	...
	void process(string key) {
		...
		if (currentMenu < 0 or not menuMask.test(currentMenu)) {
			...
			for (int i = 0; i < static_cast<int>(menuMask.size()); ++i) {
