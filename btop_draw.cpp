...
namespace Draw {
	...
	string createBox(const int x, const int y, const int width,
		...
		for (int hpos = y + 1; hpos < y + height - 1; ++hpos) {
			out += Mv::to(hpos, x) + Symbols::v_line
				+ ((fill) ? std::string(width - 2, ' ') : Mv::r(width - 2))
				+ Symbols::v_line;
		}
	...
	string Meter::operator()(int value) {
		...
		for (int i = 1; i <= width; ++i) {
			int y = static_cast<int>(std::round(static_cast<double>(i) * 100.0 / width));
			if (value >= y) {
				out += Theme::g(color_gradient).at(invert ? 100 - y : y) + Symbols::meter;
			} else {
				out += Theme::c("meter_bg") + std::string(width + 1 - i, Symbols::meter[0]);
				break;
			}
		}
		out += Fx::reset;
	...
	void Graph::_create(const deque<long long>& data, int data_offset) {
		...
		for (int i = data_offset; i < static_cast<int>(data.size()); ++i) {
			// if (tty_mode and mult and i % 2 != 0) continue;
			if (!tty_mode && mult) current = !current;
			if (i < 0) {
				data_value = 0;
				last = 0;
			} else {
				data_value = data.at(i);
				if (max_value > 0) data_value = std::clamp((data_value + offset) * 100 / max_value, 0LL, 100LL);
			}

			//? Vertical iteration over height of graph
			for (int horizon = 0; horizon < height; ++horizon) {
				const int cur_high = (height > 1) ? static_cast<int>(std::round(100.0 * (height - horizon) / height)) : 100;
				const int cur_low = (height > 1) ? static_cast<int>(std::round(100.0 * (height - (horizon + 1)) / height)) : 0;

				//? Calculate previous + current value to fit two values in 1 braille character
				std::vector<int> result(2);
				int ai = 0;
				for (const auto& value : {last, data_value}) {
					const int clamp_min = (no_zero && horizon == height - 1 && !(mult && i == data_offset && ai == 0)) ? 1 : 0;
					if (value >= cur_high)
						result[ai++] = 4;
					else if (value <= cur_low)
						result[ai++] = clamp_min;
					else {
						result[ai++] = std::clamp(static_cast<int>(std::round((float)(value - cur_low) * 4 / (cur_high - cur_low) + mod)), clamp_min, 4);
					}
				}
				...
				if (height == 1) {
					if (result.at(0) + result.at(1) == 0) {
						...
						if (!color_gradient.empty()) {
							graphs.at(current).at(horizon) += Theme::g(color_gradient).at(std::clamp(std::max(last, data_value), 0LL, 100LL));
						}
	...
	: width(width), height(height), color_gradient(color_gradient),
	  invert(invert), no_zero(no_zero), offset(offset) {
		...
		for (int i = 0; i < height * 2; ++i) {
			if (tty_mode && i % 2 != current) continue;
			graphs[(i % 2 != 0)].push_back((value_width < width) ? ((height == 1) ? Mv::r(1) : std::string(width - value_width, ' ')) : "");
		}
	...
	string& Graph::operator()(const deque<long long>& data, bool data_same) {
		...
		for (int i = 0; i < height; ++i) {
			if (height == 1 && graphs.at(current).at(i).at(1) == '[') {
...
namespace Cpu {
    ...
    string draw(const cpu_info& cpu, const vector<Gpu::gpu_info>& gpus, bool force_redraw, bool data_same) {
		...
		if (redraw) {
			...
			if (show_temps) {
				...
				if (not hide_cores and b_column_size > 1) {
					for (size_t i = 1; i < cpu.temp.size(); ++i) {
		...
		for (int n = 0; n < Shared::coreCount; ++n) {
			out += Mv::to(b_y + cy + 1, b_x + cx + 1) + Theme::c("main_fg") + (Shared::coreCount < 100 ? Fx::b + 'C' + Fx::ub : "")
				+ ljust(std::to_string(n), core_width);

			if ((b_column_size > 0 || extra_width > 0) && n < static_cast<int>(core_graphs.size())) {
				out += Theme::c("inactive_fg") + graph_bg * (5 * b_column_size + extra_width) + Mv::l(5 * b_column_size + extra_width)
					+ core_graphs.at(n)(safeVal(cpu.core_percent, n), data_same || redraw);
			}

			out += Theme::g("cpu").at(std::clamp(safeVal(cpu.core_percent, n).back(), 0LL, 100LL));
			out += rjust(std::to_string(safeVal(cpu.core_percent, n).back()), (b_column_size < 2 ? 3 : 4)) + Theme::c("main_fg") + '%';

			if (show_temps && !hide_cores) {
				auto temp_pair = celsius_to(safeVal(cpu.temp, n + 1).back(), temp_scale);
				int temp = std::get<0>(temp_pair);
				std::string unit = std::get<1>(temp_pair);
				const auto& temp_color = Theme::g("temp").at(
					std::clamp(safeVal(cpu.temp, n + 1).back() * 100 / cpu.temp_max, 0LL, 100LL)
				);

				if (b_column_size > 1 && temp_graphs.size() >= static_cast<size_t>(n + 1)) {
					out += ' ' + Theme::c("inactive_fg") + graph_bg * 5 + Mv::l(5)
						+ temp_graphs.at(n + 1)(safeVal(cpu.temp, n + 1), data_same || redraw);
				}
				out += temp_color + rjust(std::to_string(temp), 4) + Theme::c("main_fg") + unit;
			}

			out += Theme::c("div_line") + Symbols::v_line;

			if ((++cy > std::ceil(static_cast<double>(Shared::coreCount) / b_columns) || cy == b_height - 2) && n != Shared::coreCount - 1) {
					...
					for (const auto& [name, disk] : mem.disks) {
						...
						if (io_mode) {
							...
							if (io_graph_combined) {
								// Create a deque to store the combined values
								std::deque<long long> combined(disk.io_read.size(), 0);

								// Replace rng::transform with std::transform
								std::transform(disk.io_read.begin(), disk.io_read.end(), disk.io_write.begin(), combined.begin(), std::plus<long long>());

								// Assign the combined graph to io_graphs
...
namespace Proc {
	...
	string draw(const vector<proc_info>& plist, bool force_redraw, bool data_same) {
		...
		if (not data_same and ++counter >= 100) {
			...
			// Helper function to replace std::erase_if
			

			erase_if(p_graphs, [&](const auto& pair) {
				return std::find_if(plist.begin(), plist.end(), [&](const auto& proc) {
					return proc.pid == pair.first;
				}) == plist.end();
			});

			erase_if(p_counters, [&](const auto& pair) {
				return std::find_if(plist.begin(), plist.end(), [&](const auto& proc) {
					return proc.pid == pair.first;
				}) == plist.end();
			});

			erase_if(p_wide_cmd, [&](const auto& pair) {
				return std::find_if(plist.begin(), plist.end(), [&](const auto& proc) {
					return proc.pid == pair.first;
				}) == plist.end();
			});
