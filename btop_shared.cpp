...
namespace Proc {
	void proc_sorter(vector<proc_info>& proc_vec, const string& sorting, bool reverse, bool tree) {
		if (reverse) {
			switch (v_index(sort_vector, sorting)) {
				case 0:
					std::stable_sort(proc_vec.begin(), proc_vec.end(), [](const proc_info& a, const proc_info& b) {
						return a.pid < b.pid;
					});
					break;
				case 1:
	...
	void tree_sort(vector<tree_proc>& proc_vec, const string& sorting, bool reverse, int& c_index, const int index_max, bool collapsed) {
		if (proc_vec.size() > 1) {
			...
			if (reverse) {
				switch (v_index(sort_vector, sorting)) {
				case 3: std::stable_sort(proc_vec.begin(), proc_vec.end(), [](const auto& a, const auto& b) { return a.entry.get().threads < b.entry.get().threads; });	break;
				case 5: std::stable_sort(proc_vec.begin(), proc_vec.end(), [](const auto& a, const auto& b) { return a.entry.get().mem < b.entry.get().mem; });	break;
				case 6: std::stable_sort(proc_vec.begin(), proc_vec.end(), [](const auto& a, const auto& b) { return a.entry.get().cpu_p < b.entry.get().cpu_p; });	break;
				case 7: std::stable_sort(proc_vec.begin(), proc_vec.end(), [](const auto& a, const auto& b) { return a.entry.get().cpu_c < b.entry.get().cpu_c; });	break;
				}
			}
			else {
				switch (v_index(sort_vector, sorting)) {
	...
	void _tree_gen(proc_info& cur_proc, vector<proc_info>& in_procs, vector<tree_proc>& out_procs,
		...
		// Replace rng::equal_range with std::equal_range
		auto range = std::equal_range(
			in_procs.begin(), in_procs.end(), cur_proc.pid,
			[](const proc_info& lhs, const int& pid) {
				return lhs.ppid < static_cast<uint64_t>(pid);
			}
		);
