	...
	void process(const string& key) {
		...
		try {
			...
			if (Cpu::shown) {
				...
				if (key == "+" && Config::getI("update_ms") <= 86399900) {
					// Replace rng::all_of with std::all_of
					bool all_plus = std::all_of(Input::history.begin(), Input::history.end(), [](const auto& str) {
						return str == "+";
					});

					int add = (Config::getI("update_ms") <= 86399000 && last_press >= time_ms() - 200 && all_plus)
						? 1000 : 100;
				...
				else if (key == "-" && Config::getI("update_ms") >= 200) {
					// Replace rng::all_of with std::all_of
					bool all_minus = std::all_of(Input::history.begin(), Input::history.end(), [](const auto& str) {
						return str == "-";
					});

					int sub = (Config::getI("update_ms") >= 2000 && last_press >= time_ms() - 200 && all_minus)
						? 1000 : 100;
